...
現在 TS 由来なのは以下の2件。どちらも 2026-09-17 にらせんツールが端面を横切る構成へ
変えたもので、未修正 OCCT 7.9.3 の Python オラクルでは評価できない:

- `18_iso_nut` — 体積は Python 時代の値 4614.7444 に対し 4614.7431 で一致、面数は 30 → 31
- `19_iso_bolt` — **幾何が変わっている**。同じ書き直しでねじ長も半ねじ(`thread_len = 15`)
  から全ねじ(`thread_len = shank - 2 = 28`)になった。M16 は b = 2d + 6 = 38 > shank 30 で
  ISO 4017 / JIS B 1180 上は全ねじが正しく、半ねじだったのはビルド時間短縮のため
  (`devel/perf.md` 2026-09-01)。旧スナップショット(Python オラクル、volume 10449.5876 /
  27 faces)は `thread_len = 15` で完全に再現できることを確認した上で、2026-09-17 に
  全ねじの値(volume 10043.3411 / 34 faces)へ再生成した
